...
let main args =
    ...
    |"-crypt" ->
        ...
        let fileSize = IO.FileInfo(inPath).Length |> BitConverter.GetBytes
        let text = readFile inPath
        let textWithSize = Array.concat([fileSize; text])
        let textToWork = textWithSize |> tBytesSplit128
        startTime.Start()
    ...
    |"-decrypt" ->
        ...
        startTime.Start()
        let prom = List.map (decrypt keySchedule) textToWork
                |> List.concat 
                |> List.toArray
        let fileSize = BitConverter.ToInt64(prom.[..7], 0)
        prom.[8..7 + int(fileSize)]
        |> writeFile outPath
        startTime.Stop()
